...
func makeHook(out string){
	exec.Command("melt", "color:black", "out=70",
		"-filter", "pango:text='$1000 -> $10000\nحلال 100%':family=Impact:size=115:fgcolour=gold:weight=bold:align=center",
	...
	exec.Command("melt", "color:0x1a3a5f", "out=70",
		"-filter", "pango:text='السر 15 دقيقة\n3-2-1':family=Impact:size=105:fgcolour=#00ff88:weight=bold:align=center",
...
func main(){
	fmt.Println("7 SEC HOOK + 10 HIDDEN ANGLES - 15 MIN - FIXED")
	...
	for vid:=1; vid<=4; vid++{
		...
		for idx, ang := range angles {
			makeAngle(fmt.Sprintf("angle%d_%s.mp4", vid, ang), colors[idx], fmt.Sprintf("VIDEO %d - %d/8 %s", vid, idx+1, ang), ang, 110)
		}
